...
export async function onRequestGet({env}){
  const {results}=await env.DB.prepare("SELECT bar_name,suggested_tags,source FROM bar_tag_suggestions WHERE status='accepted' ORDER BY reviewed_at,created_at").all();
  const grouped={};
