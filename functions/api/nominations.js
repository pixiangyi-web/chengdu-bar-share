...
export async function onRequestPost({request,env}){
  ...
  await env.DB.prepare(`INSERT INTO bar_nominations (bar_name,area,bar_type,source_url,reason,device_hash) VALUES (?,?,?,?,?,?) ON CONFLICT(bar_name,device_hash) DO UPDATE SET area=excluded.area,bar_type=excluded.bar_type,source_url=excluded.source_url,reason=excluded.reason,status='pending',created_at=CURRENT_TIMESTAMP`).bind(barName,area,barType||null,sourceUrl||null,reason,device).run();
  return json({ok:true});
}
