...
export async function onRequestPost({request,env}){
  ...
  const barId=String(body.bar_id||"").trim(),device=String(body.device_hash||"");
  ...
  await env.DB.prepare(`INSERT INTO community_feedback (bar_id,device_hash,classic_score,special_score,environment_score,service_score,value_score,rank_opinion) VALUES (?,?,?,?,?,?,?,?) ON CONFLICT(bar_id,device_hash) DO UPDATE SET classic_score=excluded.classic_score,special_score=excluded.special_score,environment_score=excluded.environment_score,service_score=excluded.service_score,value_score=excluded.value_score,rank_opinion=excluded.rank_opinion,updated_at=CURRENT_TIMESTAMP`).bind(barId,device,...scoreFields.map(field=>body[field]),body.rank_opinion).run();
  return json({ok:true});
}
