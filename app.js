...
async function loadApprovedTags(){try{const response=await fetch('/api/tag-suggestions');if(!response.ok)return;const data=await response.json();for(const bar of bars){const approved=(data.tags?.[bar.name]||[]).filter(tag=>!(bar.name==='Glitch'&&tag==='精酿选择'));bar.features=[...approved,...bar.features.filter(tag=>!approved.includes(tag))]}renderRanking()}catch{}}
