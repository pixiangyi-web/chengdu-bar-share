...
function mobileCard(bar){return `<article class="mobile-card"><div class="mobile-card-head"><span class="mobile-rank">${bar.rank}</span><h3>${esc(bar.name)}</h3><b class="mobile-total">${formatScore(bar.scores.total)}</b></div><div class="mobile-meta"><span>${formatMoney(bar.average)}</span><span>${esc(bar.type)}</span><span>${esc(bar.area)}</span>${bar.dianpingUrl?`<a href="${esc(bar.dianpingUrl)}" target="_blank" rel="noopener">大众点评 ↗</a>`:''}</div><div class="mobile-signature"><small>代表特调</small>${signatureHtml(bar)}</div><div class="mobile-scores">${['professional','dianping','xhs','local','travel'].map(key=>`<span>${formatScore(bar.scores[key])}<small>${scoreLabels[key].replace('分','')}</small></span>`).join('')}</div>${featuresHtml(bar,5)}${rateButton(bar)}</article>`}
...
mapBars.forEach(bar=>{const drink=menuHighlights[bar.name]?.name;L.circleMarker([bar.mapLat,bar.mapLon],{radius:5,color:'#fffaf1',weight:2,fillColor:'#1c1814',fillOpacity:.9}).bindTooltip(`${bar.rank}. ${bar.name}${drink?` · 代表特调：${drink}`:''}`).on('click',()=>selectArea(bar.areaKey,false)).addTo(markerLayer)});
